...
public class NewsController : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] CreateNewsDto dto)
    {
        ...
        // Уведомляем всех жильцов о новой новости от УК (как при создании опроса).
        if (post.SourceType == "ManagementCompany")
        {
            ...
            foreach (var rid in residentIds)
            {
                _db.Notifications.Add(new Notification
                {
                    Id = Guid.NewGuid(),
                    UserId = rid,
                    Title = post.IsPinned ? "Важное объявление" : "Новое объявление",
                    Message = post.Title,
    ...
    public async Task<IActionResult> UploadAttachment(Guid id, IFormFile file)
    {
        ...
        // FileName показывается пользователю — приводим к безопасному виду,
        // отрезая путь и обрезая длину.
